# PC4/5/6 セットアップ記録（NIC 換装前まで）

*記録日: 2026-08-11*

Intel 5300 への **NIC 換装前**に完了させた作業と、**換装後に残っている作業**の記録。
PC1/2/3 は以前セットアップ済み。今回の対象は **PC4/5/6**（3 台とも TX/RX 兼用構成）。

> 基本手順は [SETUP_CSITOOL_JA.md](SETUP_CSITOOL_JA.md) を参照。
> このファイルは「PC4/5/6 で実際に何をどこまでやったか」の進捗記録＋本家手順への追記事項。
...
ls ~/tx_capture.sh ~/rx_capture.sh
service ssh status
```
